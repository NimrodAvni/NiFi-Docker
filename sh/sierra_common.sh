...
# 3 - xml file
read_xml_property () {
    xmlstarlet select --template --match "$2[@name='$1']" --value . --nl $3
}
...
edit_xml_property() {
  ...
  if [ -n "${property_value}" ]; then
    xmlstarlet edit --inplace --update "${property_xpath}[@name='${property_name}']" --value "${property_value}" ${xml_file}
  fi
}
...
# 4 - xml file
add_xml_property () {
    last_element_name=`xmlstarlet sel --template --match "$1[last()]" --value "@name" --nl $4`
